...
function Header({ name, dataUser }) {
  ...
  return (
    <header className={`header page__header${burger ? ' page__header_opened' : ''}`}>
      <img
        src={logo}
        alt="Логотип"
      ...
      {name === 'signup' || name === 'signin' ?
        ...
        <>
          <div className={`header__email-container ${burger ? 'header__email-container_opened' : ''}`}>
            <p className='header__email'>{dataUser}</p>
            <Link to={`/sign-in`} className='header__unlogin' onClick={onSignOut}>Выйти</Link>
          </div>
          <button className={`header__button ${burger ? 'header__button_active' : ''}`} onClick={handelClick}></button>
        </>
      }
    </header>
  )
